...
import '../../network/response/emiwalletpaymentresponse.dart';
import 'thank_u_payment.dart';

class EMIPaymentWallet extends StatefulWidget {
  String orderid, emiorderid;
  double balanceamount, totalpayable, emiamount, latecharge;
  int isdeduct;
  EMIPaymentWallet(this.orderid, this.balanceamount, this.totalpayable,
      this.emiorderid, this.isdeduct, this.emiamount, this.latecharge);

  State createState() => EMIPaymentWalletState();
}

class EMIPaymentWalletState extends State<EMIPaymentWallet> {
  @override
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('EMI Amount: ${widget.emiamount.inRupeesFormat()}'),
            SizedBox(
              height: 10,
            ),
            Text('Late EMI Charge: ${widget.latecharge.inRupeesFormat()}'),
            SizedBox(
              height: 10,
            ),
            Text('Total Amount:${widget.totalpayable.inRupeesFormat()}'),
            SizedBox(
              height: 30,
            ),
            ButtonDesign(
                onPressed: () {
                  EMIupdatewalletpaymentstatus(widget.orderid);
                },
                child: 'Pay'.buttoText())
          ],
        ),
  ...
  Future<void> EMIupdatewalletpaymentstatus(String orderid) async {
    try {
      ProgressDialog.showProgressDialog(context, "Loading...");
      String creatjson = CreateJson().createjsonforemiwallet(
        orderid,
        widget.isdeduct.toString(),
        widget.totalpayable.toString(),
        context,
      );
      Networkcall networkcall = Networkcall();
      List<Object?>? list = await networkcall.postMethod(
          NetworkUtility.api_update_emi_pay_through_wallet,
          NetworkUtility.api_update_emi_pay_through_wallet_url,
          creatjson,
          context);
      if (list != null) {
        Navigator.pop(context);
        List<Emiwalletpaymentresponse> response = List.from(list!);
