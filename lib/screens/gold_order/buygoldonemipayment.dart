...
import 'buy_gold_payment_wallet.dart';

String orderId = "";
double amount = 0.0;

class BuyGoldOnEmiPayment extends StatefulWidget {
  double totalpayable;
  String orderid;
  BuyGoldOnEmiPayment(this.totalpayable, this.orderid);

  @override
  State<BuyGoldOnEmiPayment> createState() => _BuyGoldOnEmiPaymentState();
}

class _BuyGoldOnEmiPaymentState extends State<BuyGoldOnEmiPayment> {
...
//Need to test
            ...
            Navigator.pushReplacement(context, MaterialPageRoute(
              builder: (context) {
                return ThankuPayment(
                    widget.totalpayable, widget.orderid, widget.orderid);
              },
            ));
            break;
          case "false":
  ...
  Future<void> Networkcallforupdatetheemipaystatuscall(
    ...
    try {
      ...
      List<Object?>? list = await networkcall.postMethod(
          ...
          NetworkUtility.api_update_the_emi_pay_status,
          createjson,
          context);
      if (list != null) {
        Navigator.pop(context);
        ...
        switch (status) {
          case "true":
            ...
            Navigator.pushReplacement(context, MaterialPageRoute(
              builder: (context) {
                return ThankuPayment(
                    amount, widget.orderid, response[0].data!.tblEmiInvoiceId!);
              },
            ));
            break;
          case "false":
  ...
  Future<void> Networkcallforplaceorderwithccavenue(String orderid) async {
    try {
      ProgressDialog.showProgressDialog(context, "Loading...");
      String creatjson = CreateJson().createjsonforccavenueapicall(
        orderid,
        context,
      );
      Networkcall networkcall = Networkcall();
      List<Object?>? list = await networkcall.postMethod(
          ...
          NetworkUtility.api_initiate_cc_avenue_url,
          creatjson,
          context);
      if (list != null) {
        Navigator.pop(context);
        ...
        switch (status) {
          case "true":
            // Navigator.pushReplacement(context, MaterialPageRoute(
            //   builder: (context) {
            //     return CCAvenuepage(response[0].initiateUrl!);
            //   },
            // ));
            Navigator.pushReplacement(context, MaterialPageRoute(
              builder: (context) {
  ...
  Future<void> Networkcallforplaceorderwithuneedswallet(String orderid) async {
    try {
      ...
      List<Object?>? list = await networkcall.postMethod(
          ...
          NetworkUtility.uneeds_wallet_emi_purchase_product_by_unwallet_url,
          creatjson,
          context);
      if (list != null) {
        Navigator.pop(context);
