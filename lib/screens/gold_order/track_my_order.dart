...
class TrackMyOrder extends StatefulWidget {
  ...
  TrackMyOrder(this.orderid, this.invoiceNumber);
  State createState() => TrackMyOrderState();
}

class TrackMyOrderState extends State<TrackMyOrder> {
  @override
  void initState() {
    // TODO: implement initState
    super.initState();
    Networkcallforplaceorder("1", NetworkUtility.place_order_api);
  }
  ...
  Future<void> Networkcallforplaceorder(String status1, String api) async {
    try {
      ...
      String createjson = CreateJson()
          .createjsonfortrackorder(widget.orderid, status1, context);
      Networkcall networkcall = Networkcall();
      List<Object?>? list = await networkcall.postMethod(
          NetworkUtility.track_order, api, createjson, context);
      if (list != null) {
        Navigator.pop(context);
        List<Trackorderresponse> response = List.from(list!);
        String status = response[0].status!;
        switch (status) {
          case "true":
            placeorder = true;
            placeorderremark = response[0].data![0].remark!;
            placeorderdate = DateFormat('dd/MM/yyyy hh:mm a')
                .format(response[0].data![0].orderStatusUpdateDate!);
            Networkcallforconfirm("2", NetworkUtility.confirm_order_api);
  ...
  Future<void> Networkcallforconfirm(String status1, String api) async {
    try {
      ...
      if (list != null) {
        ...
        switch (status) {
          case "true":
            confirmremark = response[0].data![0].remark!;
            confirmdate = DateFormat('dd/MM/yyyy hh:mm a')
                .format(response[0].data![0].orderStatusUpdateDate!);
            confirmorder = true;
            _activeCurrentStep = 1;
            Networkcallforshiped("3", NetworkUtility.shiped_order_api);
            setState(() {});
            break;
          case "false":
            setState(() {});
    ...
    try {
      ...
      if (list != null) {
        ...
        switch (status) {
          case "true":
            shipedorder = true;
            shipedremark = response[0].data![0].remark!;
            shipeddate = DateFormat('dd/MM/yyyy hh:mm a')
                .format(response[0].data![0].orderStatusUpdateDate!);

            _activeCurrentStep = 2;
            Networkcallfordeliverd("4", NetworkUtility.delivered_order_api);
    ...
    try {
      ...
      if (list != null) {
        ...
        switch (status) {
          case "true":
            deliverdorder = true;
            deliverremark = response[0].data![0].remark!;
