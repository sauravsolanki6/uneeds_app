...
List<WalletDebitHistoryDatum> walletdebithistory = [];
List<WalletDebitHistoryDatum> _searchlist = [];
bool nodata = false, _isSearching = false;

class Walletdebithistory extends StatefulWidget {
  State createState() => WalletdebithistoryState();
}

class WalletdebithistoryState extends State<Walletdebithistory> {
  @override
  void initState() {
  ...
  Future<void> Networkcallforgetwalletdebithistory() async {
    try {
      ...
      List<Object?>? list = await Networkcall().postMethod(
          NetworkUtility.api_wallet_debit_history,
          NetworkUtility.api_wallet_debit_history_url,
          createjson,
          context);
      if (list != null) {
        Navigator.pop(context);
        List<Walletdebithstoryresponse> response = List.from(list!);
        ...
        title: Row(
          ...
          children: [
            GestureDetector(
              ...
              child: Row(
                children: [
                  'Wallet Debit History'.introTitleText(context),
                ],
              ).marginOnly(top: 10),
            ),
            CircleAvatar(
                ...
                : Center(
                    ...
                    child: Column(
                      children: [
                        ...
                        Text(
                          "Looks like you haven't wallet debit history yet ",
                          style: TextStyle(
                            fontSize: 16,
                            color: AppColor.bordercolor,
                          ),
  ...
  Widget listview() {
    return ListView.builder(
      itemCount: _isSearching ? _searchlist.length : walletdebithistory.length,
                        ...
                        Text(
                          _isSearching
                              ? double.parse(_searchlist[index].amount!)
                                  .inRupeesFormat()
                              : double.parse(walletdebithistory[index].amount!)
                                  .inRupeesFormat(),
                          style: TextStyle(
                              fontSize: 12,
                              fontWeight: FontWeight.w500,
                ...
                Row(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        _isSearching
                            ? _searchlist[index].remark!
                            : walletdebithistory[index].remark!,
                        style: TextStyle(
                            fontSize: 12,
                            fontWeight: FontWeight.w500,
                            color: AppColor.bordercolor),
